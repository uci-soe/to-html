...
var template = function getTemplate (name, cb) {
  ...
  error.ifNot(t, 'Template does not exist. Please see admin to add template. Existing templates include ' + template.list().join(', '));
...
template.list = function () {
  return Object.getOwnPropertyNames(template._registry);
};
